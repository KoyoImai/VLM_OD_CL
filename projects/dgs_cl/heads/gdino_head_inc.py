...
class ContrastiveEmbed(nn.Module):
    ...
    def forward(self, visual_feat: Tensor, text_feat: Tensor,
        ...
        Args:
            visual_feat (Tensor): Visual features of all box, size = [Batch, num_query + dn_query, 256] = [Batch, 900 + 200, 256]
            text_feat (Tensor): Text features of total caption, size = [Batch, valid_text_len, 256]
...
class GroundingDINOHead_inc(GroundingDINOHead):
    ...
    def split_outputs(all_layers_cls_scores, all_layers_bbox_preds, dn_meta):
        """[dgs_cl 移植改変 2026-08-21] dn_meta=None 対応の split_outputs.

        公式フォークは mmdet 本体（dino_head.py）にこのガードを入れているが、
        本リポジトリは本体無変更のため、同一実装をここで上書きする。
        dn_meta が None のときは全クエリを matching 側として返す
        （denoising 側 None は上流 loss_by_feat のガードでスキップされる）。
        """
    ...
    def __init__(self, trunc_class, setting, **kwargs):
        self.trunc_class = trunc_class
        
        if setting == 'cur_text':  # num_class = new class
            kwargs.update(num_classes = trunc_class[1] - trunc_class[0])
        else:
            kwargs.update(num_classes = trunc_class[1])  # num_class = global_class           
    ...
    def _get_targets_single(self, cls_score: Tensor, bbox_pred: Tensor,
        ...
        bbox_pred = bbox_pred * factor  # absolute cord of the bbox

        pred_instances = InstanceData(scores=cls_score, bboxes=bbox_pred)
        # assigner and sampler
        # assigned_gt_inds = [matching_gt_id, 0, 0, ……], assigned_labels = [matching_gt_label, -1, -1, ……]

        # if gt_instances['text_token_mask'].size(0) == 0:
        ...
        pos_inds = torch.nonzero(
            assign_result.gt_inds > 0, as_tuple=False).squeeze(-1).unique()     # pos query idx(foreground)
        neg_inds = torch.nonzero(
            assign_result.gt_inds == 0, as_tuple=False).squeeze(-1).unique()    # neg query idx(background)
        pos_assigned_gt_inds = assign_result.gt_inds[pos_inds] - 1
        pos_gt_bboxes = gt_bboxes[pos_assigned_gt_inds.long(), :]

        # Major changes. The labels are 0-1 binary labels for each bbox
        # and text tokens.
        labels = gt_bboxes.new_full((num_bboxes, self.max_text_len),    # [900, 256]
                                    0,
                                    dtype=torch.float32)
        labels[pos_inds] = gt_instances.positive_maps[pos_assigned_gt_inds] # each assigned query, turn corresponding column to 1     
        label_weights = gt_bboxes.new_ones(num_bboxes)  # foreground query and background query all update
        # label_weights = torch.zeros_like(cls_score)
        # label_weights[pos_inds] = 1.0

        # bbox targets
        bbox_targets = torch.zeros_like(bbox_pred, dtype=gt_bboxes.dtype)   # [900, 4]
        bbox_weights = torch.zeros_like(bbox_pred, dtype=gt_bboxes.dtype)   
        bbox_weights[pos_inds] = 1.0    # only update the foreground query
    ...
    def _predict_by_feat_single(self,
        ...
        max_per_img = self.test_cfg.get('max_per_img', len(cls_score))  ### map_per_img=300
        img_shape = img_meta['img_shape']

        # [900, 256] -> [900, 80], remove -inf and '.' token, and map the rest from grounding to cls
        if token_positive_maps is not None:
            ...
            det_labels = indexes % num_classes      #NOTE cls_score.shape[-1] = 80 != cfg.num_class,
            bbox_index = torch.div(indexes, num_classes, rounding_mode='trunc')
    ...
    def loss(self, hidden_states: Tensor, references: List[Tensor],
        ...
        outs = self(hidden_states, references, memory_text, text_token_mask)    # forward, outs = [all_layers_outputs_classes, all_layers_outputs_corrds]  (num_decoder_layers, bs, num_queries, cls_out_channels)
