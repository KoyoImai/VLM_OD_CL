...
class GroundingDINO_inc(GroundingDINO):
    ...
    def __init__(self, 
        ...
        # [dgs_cl 移植改変 2026-08-21] 公式フォークは mmdet 本体（dino.py）に
        # dn_cfg=None を許すガードを入れているが、本リポジトリは本体無変更のため、
        # 上流 DINO.__init__ には常に有効な dn_cfg を渡して CdnQueryGenerator を
        # 構築させる（state_dict キー dn_query_generator.label_embedding が θ0 と
        # 一致し ckpt がそのまま読める）。self.dn_cfg が None のとき生成器は
        # 一切呼ばれず（pre_decoder が self.dn_cfg で分岐）、直下で凍結するため
        # 学習にも DDP にも関与しない。
        if kwargs.get('dn_cfg') is None:
            kwargs['dn_cfg'] = dict(
                label_noise_scale=0.5,
                box_noise_scale=1.0,
                group_cfg=dict(dynamic=True, num_groups=None, num_dn_queries=100))
        super().__init__(*args, **kwargs)
        if self.dn_cfg is None:
            for _p in self.dn_query_generator.parameters():
