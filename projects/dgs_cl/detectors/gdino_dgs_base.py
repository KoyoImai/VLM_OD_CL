...
class GroundingDINO_DGS_Base(GroundingDINO_inc):
    ...
    def fast_chunked_predict(self, text_prompts, token_positive_maps,
                             visual_feats, entities, batch_data_samples,
                             rescale: bool = True):
        """[dgs_cl 移植改変 2026-08-22] chunked 推論（クラス数が max_text_len を
        超えるタスク用。videogames 87 クラス等）。

        公式リリースは本メソッドを predict() から呼ぶが定義を含まない
        （公式の実験は chunked を使わないクラス数のみ）。上流
        mmdet/models/detectors/grounding_dino.py の predict() の chunked 分岐を
        そのままメソッド化した（ラベルのオフセット・cat・label_names 付与まで
        同一）。ルーティング（set_moe_task_id）は呼び出し元 predict() で
        適用済みのため、ここでは forward のみを行う。
        """
        import copy as _copy
        assert len(batch_data_samples) == 1, 'chunked は bs=1 のみ対応'
        count = 0
        results_list = []
        flat_entities = [item for lst in entities[0] for item in lst]
        ...
        for b in range(len(text_prompts[0])):
            ...
            head_inputs_dict = self.forward_transformer(
                _copy.deepcopy(visual_feats), text_dict, batch_data_samples)
        ...
        pred_instances = results_list[0].cat(results_list)
        if len(pred_instances) > 0:
            label_names = []
            for labels in pred_instances.labels:
                if labels >= len(flat_entities):
                    label_names.append('unobject')
                else:
                    label_names.append(flat_entities[labels])
            pred_instances.label_names = label_names
        batch_data_samples[0].pred_instances = pred_instances
