#!/bin/bash
# =============================================================================
# exp_051 kdB — クラスタ実行の「器」（design.md §5）。
#   蒸留適用箇所アブレーション（バッチ [4,2,2]×8・リプレイあり）。6 ドメイン逐次。
#   3 条件は別ジョブなので並列に投入できる（同時実行 4 本制限。超過分はキュー待ち）。
#
#   使い方（マスターノードで）: sbatch experiments/exp_051/sbatch_kdB.sh
...
echo "SLURM_JOB_ID = ${SLURM_JOB_ID}   CACHE = $CACHE   exp_051 kdB"
...
singularity exec --nv \
  ...
  "$SIF" bash -c "COND=kdB GPUS=4 bash /workspace/kouyou/mmdetection/experiments/exp_051/run_sequential.sh"
