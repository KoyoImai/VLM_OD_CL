#!/usr/bin/env bash
# =============================================================================
# exp_051: 蒸留適用箇所アブレーション（kdA/kdB/kdD）の 6 ドメイン逐次ドライバ
...
#
# 使い方:
#   COND=kdA bash experiments/exp_051/run_sequential.sh   # kdA|kdB|kdD
#   GPUS=4 が既定（クラスタ）。
# =============================================================================
set -euo pipefail
cd "$(dirname "$0")/../.."

COND="${COND:?COND=kdA|kdB|kdD を指定}"
case "$COND" in kdA|kdB|kdD) ;; *) echo "COND は kdA|kdB|kdD"; exit 1;; esac
GPUS="${GPUS:-4}"
