#!/bin/bash
# =============================================================================
# exp_032_a train_val.sh — コンテナ内で走る学習評価ロジック。
#   note05 の分割に対応: exp_032_a = 蒸留A（画像）+ 蒸留B（テキスト）
#   条件A（全モジュール）・L2・重み比率追従・リプレイあり。各条件は3ドメイン逐次。
#   段階（$1 / 既定 all）: A|B | all
# =============================================================================
set -euo pipefail
...
run () { bash experiments/exp_028/run_sequential_kd.sh exp_032 condA l2ratio "$1"; }
...
echo "==== exp_032_a train_val ($STAGE) 完了 ===="
