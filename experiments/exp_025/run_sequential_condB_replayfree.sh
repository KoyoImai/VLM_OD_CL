#!/usr/bin/env bash
# =============================================================================
# exp_025 逐次ドライバ: リプレイ無し逐次ファインチューニング（条件B＝特徴抽出+融合）
...
#     2. dist_train（4GPU）で学習（リプレイ無し・現在ドメインのみ・batch_size=4/GPU（下流は lr_mult=0 で凍結））
...
# =============================================================================
set -euo pipefail
cd "$(dirname "$0")/../.."   # experiments/exp_025/ -> リポジトリルート

GPUS=4
export PORT="${PORT:-29560}"          # 他ジョブ（学習29500 / per-epoch評価29520）と衝突回避
CFG=experiments/exp_025/configs
EVALCFG=experiments/exp_023/configs   # eval_{domain}.py を流用
OUT=experiments/exp_025
...
for dom in "${ORDER[@]}"; do
  WD="$OUT/condB_replayfree_${dom}_work_dir"
  echo "==================== [$dom] 学習（条件B・リプレイ無し） ===================="
  if [ -z "$prev_ckpt" ] && [ -n "$THETA0" ]; then
    echo "[$dom] t=1: θ0 を明示パスで指定: $THETA0"
    bash tools/dist_train.sh "$CFG/condB_replayfree_${dom}.py" "$GPUS" --work-dir "$WD" \
...
echo "==================== exp_025 リプレイ無し逐次FT（条件A）完了 ===================="
