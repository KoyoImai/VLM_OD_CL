#!/bin/bash
# =============================================================================
# exp_054 ewc seed=2 — クラスタ実行の「器」（design.md §3）。
#   seed 頑健性確認（6 手法 × seed 1,2 の 12 ジョブの 1 本）。6 ドメイン逐次。
#   使い方（マスターノードで）: sbatch experiments/exp_054/sbatch_ewc_s2.sh
#   ★ node03 は /local_cache が無く落ちるため除外。
#   ※ 実行は design.md の承認後（行動原理3。2026-08-31 承認済み）。
# =============================================================================
#SBATCH --job-name=exp054_ewc_s2
...
echo "SLURM_JOB_ID = ${SLURM_JOB_ID}   CACHE = $CACHE   exp_054 ewc seed=2"
...
singularity exec --nv \
  ...
  "$SIF" bash -c "SEED=2 GPUS=4 bash /workspace/kouyou/mmdetection/experiments/exp_054/run_ewc.sh"
