#!/usr/bin/env bash
# =============================================================================
# exp_054 Finetuning（リプレイなし逐次 FT）: seed 変更版の 6 ドメイン逐次ドライバ
#   config は exp_024（t1-3）/ exp_040（t4-6）の replayfree をそのまま使い、
#   上書きは randomness.seed と ckpt 保存方針（last のみ）だけ（design.md §1）。
# 使い方: SEED=1 bash experiments/exp_054/run_ft.sh
# =============================================================================
set -euo pipefail
cd "$(dirname "$0")/../.."
...
EVALDIR="$EXP/eval_ft_s${SEED}"
mkdir -p "$EVALDIR"
ORDER=(underwater electromagnetic videogames aerial microscopic documents)

train_cfg() {
  case "$1" in
    underwater|electromagnetic|videogames) echo "experiments/exp_024/configs/fullft_replayfree_$1.py" ;;
    *) echo "experiments/exp_040/configs/replayfree_$1.py" ;;
  esac
}
eval_cfg() {
  case "$1" in
...
echo "==================== exp_054 Finetuning seed=$SEED ===================="
prev=""; LEARNED=()
for i in "${!ORDER[@]}"; do
  t=$((i + 1)); dom="${ORDER[$i]}"
  WD="$EXP/ft_s${SEED}_${dom}_work_dir"; ckpt="$WD/epoch_20.pth"
  echo "==================== [t=$t/6] $dom 学習 ===================="
  if [ -f "$ckpt" ]; then
    echo "[t=$t] $ckpt があるためスキップ"
  else
    bash tools/dist_train.sh "$(train_cfg "$dom")" "$GPUS" --work-dir "$WD" \
      --cfg-options "randomness.seed=$SEED" "load_from=${prev:-$THETA0}" "${CKPT_OPTS[@]}"
  fi
...
echo "==================== exp_054 Finetuning seed=$SEED 完了 ===================="
