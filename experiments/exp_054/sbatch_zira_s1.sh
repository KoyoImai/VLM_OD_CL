#!/bin/bash
# =============================================================================
# exp_054 zira seed=1 — クラスタ実行の「器」（design.md §3）。
#   seed 頑健性確認（6 手法 × seed 1,2 の 12 ジョブの 1 本）。6 ドメイン逐次。
#   使い方（マスターノードで）: sbatch experiments/exp_054/sbatch_zira_s1.sh
#   ★ node03 は /local_cache が無く落ちるため除外。
#   ※ 実行は design.md の承認後（行動原理3。2026-08-31 承認済み）。
# =============================================================================
#SBATCH --job-name=exp054_zira_s1
...
echo "SLURM_JOB_ID = ${SLURM_JOB_ID}   CACHE = $CACHE   exp_054 zira seed=1"
...
singularity exec --nv \
  ...
  "$SIF" bash -c "METHOD=zira SEED=1 GPUS=4 bash /workspace/kouyou/mmdetection/experiments/exp_054/run_zira_dithub.sh"
