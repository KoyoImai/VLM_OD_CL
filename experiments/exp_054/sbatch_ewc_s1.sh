#!/bin/bash
# =============================================================================
# exp_054 ewc seed=1 — クラスタ実行の「器」（design.md §3）。
#   seed 頑健性確認（6 手法 × seed 1,2 の 12 ジョブの 1 本）。6 ドメイン逐次。
#   使い方（マスターノードで）: sbatch experiments/exp_054/sbatch_ewc_s1.sh
...
echo "SLURM_JOB_ID = ${SLURM_JOB_ID}   CACHE = $CACHE   exp_054 ewc seed=1"
...
singularity exec --nv \
  ...
  "$SIF" bash -c "SEED=1 GPUS=4 bash /workspace/kouyou/mmdetection/experiments/exp_054/run_ewc.sh"
