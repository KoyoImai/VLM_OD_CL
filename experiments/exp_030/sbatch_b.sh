#!/bin/bash
# =============================================================================
# exp_030_b sbatch.sh — クラスタ実行の「器」。
#   note05 の分割に対応: exp_030_b = 蒸留D（融合後）+ 蒸留E（全部）
#   exp_030_a と exp_030_b は**別ジョブなので並列に投入できる**（各2条件×43h ≒ 86h）。
#
#   使い方（マスターノードで）:
##     RUN_STAGE=D sbatch experiments/exp_030/sbatch_b.sh
#     RUN_STAGE=E sbatch experiments/exp_030/sbatch_b.sh
#     sbatch experiments/exp_030/sbatch_b.sh          # all（2条件を直列・約86h）
#
...
# =============================================================================
#SBATCH --job-name=exp030b_kdDE_condA_cos
...
singularity exec --nv \
  ...
  "$SIF" bash /workspace/kouyou/mmdetection/experiments/exp_030/train_val_b.sh "$RUN_STAGE"
