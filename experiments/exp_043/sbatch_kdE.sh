#!/bin/bash
# =============================================================================
# exp_043 kdE — クラスタ実行の「器」（design.md §5）。
#   6 ドメイン（underwater -> ... -> documents）を逐次で流す（約 90〜100 h）。
#   2 条件は**別ジョブなので並列に投入できる**（同時実行 4 ジョブ制限内）。
#
#   使い方（マスターノードで）:
#     sbatch experiments/exp_043/sbatch_kdE.sh
...
singularity exec --nv \
  ...
  "$SIF" bash -c "COND=kdE GPUS=4 bash /workspace/kouyou/mmdetection/experiments/exp_043/run_sequential.sh"
