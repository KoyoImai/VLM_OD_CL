# =============================================================================
# exp_043: 蒸留E（λ=10・バッファ4枚限定）＋ リプレイ（バッチ [4,2,2]×8） / videogames（逐次 t=3）
#
# 【自動生成】experiments/exp_043/gen_configs.py。直接編集しない。
#
# 6ドメイン逐次 underwater -> electromagnetic -> videogames -> aerial -> microscopic -> documents の 3 番目。
# 学習枠は既存 RF100 系列と同一（20 epoch / lr 1e-4 / wd 1e-4。design.md §2.1）。
# バッチ構成が本実験の核（design.md §2.2）:
#   t=3: [現在4, 汎用2, 過去2]（3 ソース、過去 2 ドメイン累積）
...
_root = '/workspace/kouyou/datasets/rf100_domain/videogames/'
...
# 過去プール（学習済み 2 ドメイン × 500 を入れ子 ConcatDataset で 1 ソースに。
# バッチにはここからランダムに 2 枚入る）
#   underwater 500 + electromagnetic 500
past_pool = dict(
    type='ConcatDataset',
    datasets=[
        dict(
