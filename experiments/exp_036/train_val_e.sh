#!/bin/bash
# =============================================================================
# exp_036_e train_val.sh — 条件A + L2 蒸留 + リプレイ / **λ = 5.0 固定**
...
w = Config.fromfile(
    'experiments/exp_036/configs/kdA_condA_l2w50_underwater.py').model.kd.loss_weight
lam = float(os.environ['LAMBDA'])
assert w == lam, f'config の loss_weight={w} と LAMBDA={lam} が食い違っています'
print(f'[exp_036_e] 実効 λ = {lam}（config と一致）')
PY
...
echo "==== exp_036_e train_val ($STAGE) 完了 ===="
