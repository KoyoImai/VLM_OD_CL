#!/bin/bash
# =============================================================================
# exp_036_e sbatch.sh — クラスタ実行の「器」。
#   note07 の分割に対応: exp_036_e = 蒸留A（画像）+ 蒸留B（テキスト）、λ = 5.0
#   exp_036 の6ジョブは独立に投入できる（各2条件×32h ≒ 64h）。同時実行上限は4。
#
#   使い方（マスターノードで）:
#     RUN_STAGE=A sbatch experiments/exp_036/sbatch_e.sh
#     RUN_STAGE=B sbatch experiments/exp_036/sbatch_e.sh
#     sbatch experiments/exp_036/sbatch_e.sh          # all（2条件を直列・約64h）
#
...
# =============================================================================
#SBATCH --job-name=exp036e_kdAB_condA_l2w50
...
singularity exec --nv \
  ...
  "$SIF" bash /workspace/kouyou/mmdetection/experiments/exp_036/train_val_e.sh "$RUN_STAGE"
