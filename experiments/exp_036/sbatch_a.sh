#!/bin/bash
# =============================================================================
# exp_036_a sbatch.sh — クラスタ実行の「器」。
#   note07 の分割に対応: exp_036_a = 蒸留A（画像）+ 蒸留B（テキスト）、λ = 2.0
...
#     sbatch experiments/exp_036/sbatch_a.sh          # all（2条件を直列・約64h）
#
...
# =============================================================================
#SBATCH --job-name=exp036a_kdAB_condA_l2w20
...
singularity exec --nv \
  ...
  "$SIF" bash /workspace/kouyou/mmdetection/experiments/exp_036/train_val_a.sh "$RUN_STAGE"
