#!/bin/bash
# =============================================================================
# exp_036_a train_val.sh — 条件A + L2 蒸留 + リプレイ / **λ = 2.0 固定**
...
#   LAMBDA を設定しないと config の loss_weight=2.0 が 1.0 で上書きされる。
...
w = Config.fromfile(
    'experiments/exp_036/configs/kdA_condA_l2w20_underwater.py').model.kd.loss_weight
lam = float(os.environ['LAMBDA'])
assert w == lam, f'config の loss_weight={w} と LAMBDA={lam} が食い違っています'
print(f'[exp_036_a] 実効 λ = {lam}（config と一致）')
PY

run () { bash experiments/exp_028/run_sequential_kd.sh exp_036 condA l2w20 "$1"; }
...
echo "==== exp_036_a train_val ($STAGE) 完了 ===="
