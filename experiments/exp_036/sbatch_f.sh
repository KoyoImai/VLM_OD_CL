#!/bin/bash
# =============================================================================
# exp_036_f sbatch.sh — クラスタ実行の「器」。
#   note07 の分割に対応: exp_036_f = 蒸留D（融合後）+ 蒸留E（3項平均）、λ = 5.0
#   exp_036 の6ジョブは独立に投入できる（各2条件×32h ≒ 64h）。同時実行上限は4。
#
#   使い方（マスターノードで）:
#     RUN_STAGE=D sbatch experiments/exp_036/sbatch_f.sh
#     RUN_STAGE=E sbatch experiments/exp_036/sbatch_f.sh
#     sbatch experiments/exp_036/sbatch_f.sh          # all（2条件を直列・約64h）
#
...
# =============================================================================
#SBATCH --job-name=exp036f_kdDE_condA_l2w50
...
singularity exec --nv \
  ...
  "$SIF" bash /workspace/kouyou/mmdetection/experiments/exp_036/train_val_f.sh "$RUN_STAGE"
