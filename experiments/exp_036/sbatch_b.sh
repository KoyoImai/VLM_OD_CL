#!/bin/bash
# =============================================================================
# exp_036_b sbatch.sh — クラスタ実行の「器」。
#   note07 の分割に対応: exp_036_b = 蒸留D（融合後）+ 蒸留E（3項平均）、λ = 2.0
...
#     sbatch experiments/exp_036/sbatch_b.sh          # all（2条件を直列・約64h）
#
...
# =============================================================================
#SBATCH --job-name=exp036b_kdDE_condA_l2w20
...
singularity exec --nv \
  ...
  "$SIF" bash /workspace/kouyou/mmdetection/experiments/exp_036/train_val_b.sh "$RUN_STAGE"
