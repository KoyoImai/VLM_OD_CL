#!/usr/bin/env python3
# =============================================================================
# exp_013: 特徴抽出部（投影アダプタ含む）を θ0 に戻したハイブリッドを生成する。
#   - 学習は一切行わない。既存 state_dict を後付けで合成するのみ。
#   - imgfeat: backbone.(Swin) + neck.(ChannelMapper) を θ0 に戻す（テキスト側・検出経路は unfrozen）
#   - txtfeat: language_model.(BERT) + text_feat_map. を θ0 に戻す（画像側・検出経路は unfrozen）
#   - language_model は θ0 のみ ...position_ids（定数バッファ）あり→ 共通キーのみ置換。
# 出力: experiments/exp_013/hybrids/{domain}_{group}_theta0.pth
# 参照: experiments/exp_013/design.md
# =============================================================================
import os
import copy
import torch
...
# ロールバックするモジュール接頭辞のグループ
GROUPS = {
    'imgfeat': ('backbone.', 'neck.'),                 # 画像特徴抽出部
    'txtfeat': ('language_model.', 'text_feat_map.'),  # テキスト特徴抽出部
}
OUT_DIR = os.path.join(REPO, 'experiments/exp_013/hybrids')
...
def main():
    ...
    for d, rel in THETA1_U.items():
        ...
        for gname, prefixes in GROUPS.items():
            # 置換対象 = unfrozen 側に存在し、かつ θ0 にも存在する対象接頭辞キー（共通キー）
            ...
            only0 = [k for k in sd0 if in_group(k, prefixes) and k not in sd_u]  # 定数バッファ想定
