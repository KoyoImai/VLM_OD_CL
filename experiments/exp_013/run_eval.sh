#!/usr/bin/env bash
# =============================================================================
# exp_013 評価駆動スクリプト（exp_012 と同一形式）
#   - 対象: hybrids/{domain}_{imgfeat|txtfeat}_theta0.pth（make_hybrid_rollback.py で生成）
#   - ZCOCO: configs/mm_grounding_dino/eval_base_coco.py（(800,1333) keep_ratio・4GPU）
#   - 適応 : 各ドメイン FT config（自ドメイン valid・(800,1333)・4GPU）
#   - 出力 : experiments/exp_013/{domain}_{group}_theta0_{coco|adapt}/
#   - ログ : experiments/exp_013/{zcoco|adapt}_eval.log（exp_012 と同じ開始行書式）
#
# 使い方（リポジトリルートで実行）:
#   bash experiments/exp_013/run_eval.sh zcoco   # ZCOCO のみ
#   bash experiments/exp_013/run_eval.sh adapt   # 適応のみ
#   bash experiments/exp_013/run_eval.sh all      # 両方（既定）
#
# 注: 学習は一切行わない（forward のみ）。承認後に実行する（design.md 承認ゲート）。
# 参照: experiments/exp_013/design.md / exp_012 の zcoco_eval.log・adapt_eval.log
# =============================================================================
# 注: set -e は使わない。1本の eval 失敗で残り（最大24本）を巻き込まないため、
#     継続実行＋失敗追跡（FAILED 配列）方式にする。未定義変数のみ検出する。
set -uo pipefail
...
run_zcoco() {
  echo "[$(ts)] exp_013 hybrid ZCOCO 評価 開始" | tee -a "$ZCOCO_LOG"
  for d in "${DOMAINS[@]}"; do
  ...
  echo "[$(ts)] exp_013 hybrid ZCOCO 評価 完了" | tee -a "$ZCOCO_LOG"
}

run_adapt() {
  echo "[$(ts)] exp_013 hybrid 適応評価 開始" | tee -a "$ADAPT_LOG"
  for d in "${DOMAINS[@]}"; do
  ...
  echo "[$(ts)] exp_013 hybrid 適応評価 完了" | tee -a "$ADAPT_LOG"
}
