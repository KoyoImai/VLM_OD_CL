#!/bin/bash
# =============================================================================
# exp_041 dithub / replay — クラスタ実行の「器」（design.md §5）。
#   後半3ドメイン（aerial -> microscopic -> documents）を逐次で流す。
#   4条件は**別ジョブなので並列に投入できる**（同時実行4ジョブ制限内。design.md §5）。
#
#   使い方（マスターノードで）:
#     sbatch experiments/exp_041/sbatch_dithub_replay.sh
...
# =============================================================================
#SBATCH --job-name=exp041_dithub_replay
...
echo "SLURM_JOB_ID = ${SLURM_JOB_ID}   CACHE = $CACHE   METHOD=dithub REPLAY=replay"
...
singularity exec --nv \
  ...
  "$SIF" bash -c "METHOD=dithub REPLAY=replay GPUS=4 bash /workspace/kouyou/mmdetection/experiments/exp_041/run_sequential.sh"
