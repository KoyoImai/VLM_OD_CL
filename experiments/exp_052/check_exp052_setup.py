"""exp_052 実行前検証（design.md §4）。

使い方:
    python experiments/exp_052/check_exp052_setup.py          # config のみ（GPU 不要）
    python experiments/exp_052/check_exp052_setup.py --step   # + 実データ 1 step（GPU 1 枚）

検証項目
  1. 15 本の config が build でき、model と train_dataloader が exp_035 kdE と完全一致
...
if __name__ == '__main__':
    ...
    for cond in CONDS:
        for d in DOMAINS:
            ...
            ref = Config.fromfile(os.path.join(
                ROOT, 'experiments', 'exp_035', 'configs',
                f'kdE_condA_l2w100_{d}.py'))
            if _plain(ref.model) != _plain(c.model):
                bad.append(f'{cond}_{d}: model が exp_035 kdE と不一致')
            if _plain(ref.train_dataloader) != _plain(c.train_dataloader):
                bad.append(f'{cond}_{d}: train_dataloader が exp_035 kdE と不一致')
    ...
    check(1, '15 本 build・model/dataloader が exp_035 kdE と一致・schedule/ckpt 方針',
    ...
    c = copy.deepcopy(cfgs[('qsel', 'underwater')])
    ...
    c2 = copy.deepcopy(cfgs[('qsel', 'underwater')])
    ...
    if do_step:
        ...
        loader = Runner.build_dataloader(
            copy.deepcopy(cfgs[('qsel', 'underwater')].train_dataloader))
