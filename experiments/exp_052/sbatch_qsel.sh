#!/bin/bash
# =============================================================================
# exp_052 qsel — クラスタ実行の「器」（design.md §5）。
#   蒸留E × 学習可能モジュールアブレーション（旧バッチ [4,1,1]×6・リプレイあり）。
#   前半 3 ドメイン逐次。5 条件は別ジョブ（同時実行 4 本制限。超過分はキュー待ち）。
#
#   使い方（マスターノードで）: sbatch experiments/exp_052/sbatch_qsel.sh
...
echo "SLURM_JOB_ID = ${SLURM_JOB_ID}   CACHE = $CACHE   exp_052 qsel"
...
singularity exec --nv \
  ...
  "$SIF" bash -c "COND=qsel GPUS=4 bash /workspace/kouyou/mmdetection/experiments/exp_052/run_sequential.sh"
