#!/usr/bin/env bash
# =============================================================================
# exp_027-1 逐次ドライバ: 逐次FT + リプレイ（条件A＝全モジュール）
...
#
#   config は exp_023 の設定を継承したエイリアス（experiments/exp_027/configs/）。
...
GPUS=4
export PORT="${PORT:-29620}"          # exp_024(29540)/exp_025(29560)/exp_026(29580) と衝突回避
...
for dom in "${ORDER[@]}"; do
  WD="$OUT/fullft_replay_${dom}_work_dir"
  echo "==================== [027-1][$dom] 学習（条件A・リプレイあり） ===================="
  ...
  echo "[027-1][$dom] last ckpt: $last"

  # 現ドメイン + 全過去ドメイン（ドメイン別。平均は取らない）
  for evd in "$dom" "${PAST[@]}"; do
    echo "-------------------- [027-1][$dom] 評価 on $evd --------------------"
    bash tools/dist_test.sh "$EVALCFG/eval_${evd}.py" "$last" "$GPUS" \
      --work-dir "$OUT/fullft_replay_eval_after_${dom}/on_${evd}"
  done

  # ZCOCO（COCO2017-val zero-shot）
  echo "-------------------- [027-1][$dom] ZCOCO --------------------"
  bash tools/dist_test.sh "$ZCOCO_CFG" "$last" "$GPUS" \
    --work-dir "$OUT/fullft_replay_eval_after_${dom}/zcoco"

  prev_ckpt="$last"
  PAST+=("$dom")
done

echo "==================== exp_027-1 逐次FT + リプレイ（条件A）完了 ===================="
