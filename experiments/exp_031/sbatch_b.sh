#!/bin/bash
# =============================================================================
# exp_031_b sbatch.sh — クラスタ実行の「器」。
#   note05 の分割に対応: exp_031_b = 蒸留D（融合後）+ 蒸留E（全部）
#   exp_031_a と exp_031_b は**別ジョブなので並列に投入できる**（各2条件×43h ≒ 86h）。
#
#   使い方（マスターノードで）:
##     RUN_STAGE=D sbatch experiments/exp_031/sbatch_b.sh
#     RUN_STAGE=E sbatch experiments/exp_031/sbatch_b.sh
#     sbatch experiments/exp_031/sbatch_b.sh          # all（2条件を直列・約86h）
#
...
# =============================================================================
#SBATCH --job-name=exp031b_kdDE_condB_cos
...
singularity exec --nv \
  ...
  "$SIF" bash /workspace/kouyou/mmdetection/experiments/exp_031/train_val_b.sh "$RUN_STAGE"
