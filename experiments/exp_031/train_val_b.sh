#!/bin/bash
# =============================================================================
# exp_031_b train_val.sh — コンテナ内で走る学習評価ロジック。
#   note05 の分割に対応: exp_031_b = 蒸留D（融合後）+ 蒸留E（全部）
#   条件B（特徴抽出+融合）・コサイン類似度・リプレイあり。各条件は3ドメイン逐次。
#   段階（$1 / 既定 all）: D|E | all
# =============================================================================
set -euo pipefail
...
run () { bash experiments/exp_028/run_sequential_kd.sh exp_031 condB cos "$1"; }
...
echo "==== exp_031_b train_val ($STAGE) 完了 ===="
