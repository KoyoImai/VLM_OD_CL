#!/bin/bash
# =============================================================================
# exp_045 inflora — クラスタ実行の「器」（design.md §5）。
#   RF100 6 ドメイン逐次・バッファ不使用（約 30〜40 h）。
...
# =============================================================================
#SBATCH --job-name=exp045_inflora
...
# 作業ディレクトリは exp_045 専用クローン（design.md §5。2026-08-16 前提）
REPO="${REPO:-/home/kouyou/VLM_OD_CL_exp045}"
...
echo "SLURM_JOB_ID = ${SLURM_JOB_ID}   CACHE = $CACHE   exp_045 inflora"
...
singularity exec --nv \
  ...
  "$SIF" bash -c "GPUS=4 bash /workspace/kouyou/mmdetection/experiments/exp_045/run_inflora.sh"
