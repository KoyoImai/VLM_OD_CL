#!/bin/bash
# =============================================================================
# exp_045 ewc — クラスタ実行の「器」（design.md §5）。
#   RF100 6 ドメイン逐次・バッファ不使用（約 40〜45 h）。
...
echo "SLURM_JOB_ID = ${SLURM_JOB_ID}   CACHE = $CACHE   exp_045 ewc"
...
singularity exec --nv \
  ...
  "$SIF" bash -c "LAM=1000 GPUS=4 bash /workspace/kouyou/mmdetection/experiments/exp_045/run_ewc.sh"
