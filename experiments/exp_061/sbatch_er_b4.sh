...
echo "SLURM_JOB_ID=${SLURM_JOB_ID} CACHE=$CACHE  exp_061 er b4"
singularity exec --nv \
  ...
  "$SIF" bash -c "METHOD=er COND=b4 GPUS=4 bash /workspace/kouyou/mmdetection/experiments/exp_061/run_sequential.sh"
