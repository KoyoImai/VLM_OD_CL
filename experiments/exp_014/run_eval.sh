#!/usr/bin/env bash
# =============================================================================
# exp_014 評価駆動スクリプト（exp_013 と同一形式）
#   - 対象: hybrids/{domain}_{enc|fusion|text|image}_theta0.pth（make_hybrid_encoder.py で生成）
#   - ZCOCO: configs/mm_grounding_dino/eval_base_coco.py（(800,1333) keep_ratio・4GPU）
#   - 適応 : 各ドメイン FT config（自ドメイン valid・(800,1333)・4GPU）
#   - 出力 : experiments/exp_014/{domain}_{group}_theta0_{coco|adapt}/
#   - ログ : experiments/exp_014/{zcoco|adapt}_eval.log
#
# 使い方（リポジトリルートで実行）:
#   bash experiments/exp_014/run_eval.sh zcoco   # ZCOCO のみ
#   bash experiments/exp_014/run_eval.sh adapt   # 適応のみ
#   bash experiments/exp_014/run_eval.sh all      # 両方（既定）
#
# 注: 学習は一切行わない（forward のみ）。承認後に実行する（design.md 承認ゲート）。
#     GROUPS は bash 特殊変数のため使わない（MOD_GROUPS を使用）。
# =============================================================================
# set -e は使わない。1本の eval 失敗で残り（最大48本）を巻き込まないため継続実行＋失敗追跡。
set -uo pipefail
...
run_zcoco() {
  echo "[$(ts)] exp_014 hybrid ZCOCO 評価 開始" | tee -a "$ZCOCO_LOG"
  for d in "${DOMAINS[@]}"; do
  ...
  echo "[$(ts)] exp_014 hybrid ZCOCO 評価 完了" | tee -a "$ZCOCO_LOG"
}

run_adapt() {
  echo "[$(ts)] exp_014 hybrid 適応評価 開始" | tee -a "$ADAPT_LOG"
  for d in "${DOMAINS[@]}"; do
  ...
  echo "[$(ts)] exp_014 hybrid 適応評価 完了" | tee -a "$ADAPT_LOG"
}
