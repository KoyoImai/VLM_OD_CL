#!/bin/bash
# =============================================================================
# exp_028_b train_val.sh — コンテナ内で走る学習評価ロジック。
#   note05 の分割に対応: exp_028_b = 蒸留D（融合後）+ 蒸留E（全部）
#   条件A（全モジュール）・L2ノルム・リプレイあり。各蒸留条件は3ドメイン逐次（約43h）。
#   段階（$1 / 既定 all）: D|E | all
# =============================================================================
set -euo pipefail
...
run () { bash experiments/exp_028/run_sequential_kd.sh exp_028 condA l2 "$1"; }
...
echo "==== exp_028_b train_val ($STAGE) 完了 ===="
