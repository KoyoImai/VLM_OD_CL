#!/bin/bash
# =============================================================================
# exp_028_a sbatch.sh — クラスタ実行の「器」。
#   note05 の分割に対応: exp_028_a = 蒸留A（画像）+ 蒸留B（テキスト）
#   exp_028_a と exp_028_b は**別ジョブなので並列に投入できる**（各2条件×43h ≒ 86h）。
#
#   使い方（マスターノードで）:
##     RUN_STAGE=A sbatch experiments/exp_028/sbatch_a.sh
#     RUN_STAGE=B sbatch experiments/exp_028/sbatch_a.sh
#     sbatch experiments/exp_028/sbatch_a.sh          # all（2条件を直列・約86h）
#
...
# =============================================================================
#SBATCH --job-name=exp028a_kdAB_condA_l2
...
singularity exec --nv \
  ...
  "$SIF" bash /workspace/kouyou/mmdetection/experiments/exp_028/train_val_a.sh "$RUN_STAGE"
