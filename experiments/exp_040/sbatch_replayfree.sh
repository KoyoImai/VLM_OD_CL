...
echo "SLURM_JOB_ID = ${SLURM_JOB_ID}   CACHE = $CACHE   COND = replayfree"
...
singularity exec --nv \
  ...
  "$SIF" bash -c "COND=replayfree GPUS=4 bash /workspace/kouyou/mmdetection/experiments/exp_040/run_sequential.sh"
