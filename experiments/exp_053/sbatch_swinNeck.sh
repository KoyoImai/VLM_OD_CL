#!/bin/bash
# =============================================================================
# exp_053 swinNeck — クラスタ実行の「器」（design.md §3）。
#   蒸留E × 学習可能モジュールアブレーションの後半 3 ドメイン継続（t=4..6）。
#   exp_052 クローンの experiments/exp_052 を read-only bind し、t=4 の初期重みに
#   exp_052 同一条件の videogames epoch_20 を使う（design.md §1.2）。
#
#   使い方（マスターノードで）: sbatch experiments/exp_053/sbatch_swinNeck.sh
#   ★ 投入前に exp_052 側の ckpt 存在を確認すること（README §2）。
#   ★ リプレイに Objects365、ZCOCO に MSCOCO の bind が必要。
#   ★ node03 は /local_cache が用意されず mkdir で落ちるため除外する。
#   ※ 実行は design.md の承認後（行動原理3。2026-08-25 承認済み）。
# =============================================================================
#SBATCH --job-name=exp053_swinNeck
...
REPO="${REPO:-/home/kouyou/VLM_OD_CL_exp053}"
REPO52="${REPO52:-/home/kouyou/VLM_OD_CL_exp052}"
...
echo "SLURM_JOB_ID = ${SLURM_JOB_ID}   CACHE = $CACHE   exp_053 swinNeck"
...
singularity exec --nv \
  ...
  --bind "$REPO52/experiments/exp_052":/workspace/kouyou/mmdetection/experiments/exp_052:ro \
  ...
  "$SIF" bash -c "COND=swinNeck GPUS=4 bash /workspace/kouyou/mmdetection/experiments/exp_053/run_sequential.sh"
