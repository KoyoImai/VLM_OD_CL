#!/bin/bash
# exp_062 ours base — クラスタ実行の器（design.md 承認後）。
#   バッチ内訳感度 [4,2,2]。6 ドメイン逐次。
#   ★ config は exp_058/buffer を参照する。クローンに exp_058/buffer を必ず含めること（README §0）。
#SBATCH --job-name=exp062_ours_base
...
REPO="${REPO:-/home/kouyou/VLM_OD_CL_exp062}"
...
echo "SLURM_JOB_ID=${SLURM_JOB_ID} CACHE=$CACHE  exp_062 ours base"
singularity exec --nv \
  ...
  "$SIF" bash -c "METHOD=ours COND=base GPUS=4 bash /workspace/kouyou/mmdetection/experiments/exp_062/run_sequential.sh"
