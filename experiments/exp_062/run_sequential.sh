#!/usr/bin/env bash
# =============================================================================
# exp_062: バッチ内訳感度（[4,2,2]） の逐次ドライバ
#   Ours(kdE)/ER(replay) 両対応。ER は teacher_ckpt を持たないため load_from のみ渡す。
# 使い方: METHOD=ours|er COND=base|b2|b3|b4|b5 GPUS=4 bash experiments/exp_062/run_sequential.sh
# =============================================================================
set -euo pipefail
cd "$(dirname "$0")/../.."
METHOD="${METHOD:?METHOD=ours|er を指定}"
case "$METHOD" in ours|er) ;; *) echo "METHOD は ours|er"; exit 1;; esac
COND="${COND:?COND=base|b2|b3|b4|b5 を指定}"
case "$COND" in base|b2|b3|b4|b5) ;; *) echo "COND は base|b2|b3|b4|b5"; exit 1;; esac
GPUS="${GPUS:-4}"
if [ -z "${THETA0:-}" ]; then
  for p in \
