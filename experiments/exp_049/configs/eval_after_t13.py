# =============================================================================
# exp_049: DGS 評価（t=13 時点・学習済み 13 タスク per-dataset）
...
val_evaluator_pistols = dict(
    type='CocoMetric',
    ann_file='data/odinw/pistols/export/test_annotations_without_background.json',
    metric='bbox')
...
val_evaluator_Raccoon = dict(
    type='CocoMetric',
    ann_file='data/odinw/Raccoon/Raccoon.v38-416x416-resize.coco/test/annotations_without_background.json',
    metric='bbox')

# --------------------- ShellfishOpenImages ---------------------#
dataset_ShellfishOpenImages = dict(
    type='CocoDataset',
    metainfo=dict(classes=('Crab', 'Lobster', 'Shrimp')),
...
val_evaluator_ShellfishOpenImages = dict(
    type='CocoMetric',
    ann_file='data/odinw/ShellfishOpenImages/416x416/test/annotations_without_background.json',
    metric='bbox')
...
val_evaluator_thermalDogsAndPeople = dict(
    type='CocoMetric',
    ann_file='data/odinw/thermalDogsAndPeople/test/annotations_without_background.json',
    metric='bbox')

# --------------------- VehiclesOpenImages ---------------------#
dataset_VehiclesOpenImages = dict(
    type='CocoDataset',
    metainfo=dict(classes=('Ambulance', 'Bus', 'Car', 'Motorcycle', 'Truck')),
...
val_evaluator_VehiclesOpenImages = dict(
    type='CocoMetric',
    ann_file='data/odinw/VehiclesOpenImages/416x416/test/annotations_without_background.json',
    metric='bbox')

val_dataloader = dict(
    batch_size=1,
    dataset=dict(_delete_=True, type='ConcatDataset', datasets=[dataset_AerialMaritimeDrone, dataset_Aquarium, dataset_CottontailRabbits, dataset_EgoHands, dataset_NorthAmericaMushroom, dataset_Packages, dataset_PascalVOC, dataset_pistols, dataset_pothole, dataset_Raccoon, dataset_ShellfishOpenImages, dataset_thermalDogsAndPeople, dataset_VehiclesOpenImages]))
test_dataloader = val_dataloader

val_evaluator = dict(
    _delete_=True,
    type='MultiDatasetsEvaluator',
    metrics=[val_evaluator_AerialMaritimeDrone, val_evaluator_Aquarium, val_evaluator_CottontailRabbits, val_evaluator_EgoHands, val_evaluator_NorthAmericaMushroom, val_evaluator_Packages, val_evaluator_PascalVOC, val_evaluator_pistols, val_evaluator_pothole, val_evaluator_Raccoon, val_evaluator_ShellfishOpenImages, val_evaluator_thermalDogsAndPeople, val_evaluator_VehiclesOpenImages],
    dataset_prefixes=['AerialMaritimeDrone', 'Aquarium', 'CottontailRabbits', 'EgoHands', 'NorthAmericaMushroom', 'Packages', 'PascalVOC', 'pistols', 'pothole', 'Raccoon', 'ShellfishOpenImages', 'thermalDogsAndPeople', 'VehiclesOpenImages'])
