#!/usr/bin/env bash
# =============================================================================
# exp_048 条件1 kdEonly: リプレイ無し・現在ドメイン全4枚に蒸留E の 6 ドメイン逐次ドライバ
...
#
# 使い方:
#   bash experiments/exp_048/run_kdEonly.sh      # GPUS=4 が既定（クラスタ）
# =============================================================================
set -euo pipefail
cd "$(dirname "$0")/../.."
...
for i in "${!ORDER[@]}"; do
  ...
  if [ -f "$ckpt" ]; then
    ...
    bash tools/dist_train.sh "$CFG/kdEonly_${dom}.py" "$GPUS" \
...
echo "==================== exp_048 kdEonly 完了（最終: $prev）===================="
