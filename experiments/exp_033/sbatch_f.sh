#!/bin/bash
# =============================================================================
# exp_033_a sbatch.sh — クラスタ実行の「器」。
#   note05 の分割に対応: exp_033_a = 蒸留A（画像）+ 蒸留B（テキスト）
#   exp_033_a と exp_033_b は**別ジョブなので並列に投入できる**（各2条件×43h ≒ 86h）。
#
#   使い方（マスターノードで）:
##     RUN_STAGE=A sbatch experiments/exp_033/sbatch_a.sh
#     RUN_STAGE=B sbatch experiments/exp_033/sbatch_a.sh
#     sbatch experiments/exp_033/sbatch_a.sh          # all（2条件を直列・約86h）
#
...
# =============================================================================
#SBATCH --job-name=exp033f_kdDE_condA_l2w50
...
singularity exec --nv \
  ...
  "$SIF" bash /workspace/kouyou/mmdetection/experiments/exp_033/train_val_f.sh "$RUN_STAGE"
