#!/bin/bash
# =============================================================================
# exp_039 DitHub — クラスタ実行の「器」（design.md §5）。
#   リプレイ無し → 有り を直列に流す（約 54 h）。ZiRa は sbatch_dithub.sh で
#   **別ジョブとして並列に投入できる**（同時実行 4 ジョブ制限内）。
#
#   使い方（マスターノードで）:
#     sbatch experiments/exp_039/sbatch_dithub.sh                    # 2 条件を直列
#     RUN_STAGE=replayfree sbatch experiments/exp_039/sbatch_dithub.sh
#     RUN_STAGE=replay     sbatch experiments/exp_039/sbatch_dithub.sh
#
#   ★ リプレイを使うため Objects365 の bind が必要（exp_027 / exp_028 と同じ）。
#   ★ node03 は /local_cache が用意されず mkdir で落ちるため除外する。
#   ※ 実行は design.md の承認後（行動原理3）。
# =============================================================================
#SBATCH --job-name=exp039_dithub
