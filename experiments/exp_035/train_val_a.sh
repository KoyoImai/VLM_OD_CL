#!/bin/bash
# =============================================================================
# exp_035_a train_val.sh — 条件A + L2 蒸留 + リプレイ / **λ = 10.0 固定**
...
#   LAMBDA を設定しないと config の loss_weight=10.0 が 1.0 で上書きされる。
...
w = Config.fromfile(
    'experiments/exp_035/configs/kdA_condA_l2w100_underwater.py').model.kd.loss_weight
lam = float(os.environ['LAMBDA'])
assert w == lam, f'config の loss_weight={w} と LAMBDA={lam} が食い違っています'
print(f'[exp_035] 実効 λ = {lam}（config と一致）')
PY

run () { bash experiments/exp_028/run_sequential_kd.sh exp_035 condA l2w100 "$1"; }
...
echo "==== exp_035_a train_val ($STAGE) 完了 ===="
