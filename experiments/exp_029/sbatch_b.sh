#!/bin/bash
# =============================================================================
# exp_029_b sbatch.sh — クラスタ実行の「器」。
#   note05 の分割に対応: exp_029_b = 蒸留D（融合後）+ 蒸留E（全部）
#   exp_029_a と exp_029_b は**別ジョブなので並列に投入できる**（各2条件×43h ≒ 86h）。
#
#   使い方（マスターノードで）:
##     RUN_STAGE=D sbatch experiments/exp_029/sbatch_b.sh
#     RUN_STAGE=E sbatch experiments/exp_029/sbatch_b.sh
#     sbatch experiments/exp_029/sbatch_b.sh          # all（2条件を直列・約86h）
#
...
# =============================================================================
#SBATCH --job-name=exp029b_kdDE_condB_l2
...
singularity exec --nv \
  ...
  "$SIF" bash /workspace/kouyou/mmdetection/experiments/exp_029/train_val_b.sh "$RUN_STAGE"
